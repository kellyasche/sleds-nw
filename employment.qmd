...
Now I'm going to prep the employment data in order to join with the master dataset. So, let's import the original data. I will then filter so that I only have PersonID that have graduated from a high school in Northwest MN. The "Employment-Northwest.csv" file came from a script within that "Employment" folder, where I imported the very large employment dataset and then filtered the PersonID by Southern MN grads.
...
-   PersonID: Every individual that graduated from a Southern MN high school and was found within the large employment spreadsheet.
...
-   OwnershipCode: A classification system based upon the controlling interest of the establishment primarily designed to distinguish between governmental and non-governmental establishments
    ...
    -   30 = Local government
    -   50 = Private
-   EmployerLocationTotalWagesPaid: The total wages paid to all workers employed by an organization at a specific location during a specific calendar quarter
-   QuarterlyAverageEmployment: The average number of workers employed by an organization at a specific location during a specific calendar quarter
...
-   City: this is blank so I will eliminate
...
This dataset has `r comma(nrow(meaningful.employer))` rows which represents an PersonID and EmploymentOrganizationID with which they worked 1,000 or more hours had been worked in any given year of employment with that organization. Each PersonID is also in our master datalist. If the EmploymentOrganizationID is vacant, then it means they have a MN employment record but didn't reach the 1,000 hour threshold.
...
grad.locations <- read_csv("Data/SLEDS/Masters/Master-10.csv") %>%
  mutate(planning.region = "Northwest") %>%
...
grad.emp.location.match <- grad.locations %>%
  ...
  mutate(county.match = ifelse(employment.county == hs.county.name, 1,
         ...
         region.match = ifelse(emp.pr %in% c("Northwest"), 1,
                               ifelse(emp.edr == "Unknown", 0, -1)),
         state.match = 1) %>% 
...
It shows that 75% of the individuals had a MN employment record at their graduation year. That percentage drops to 60% two years after graduation and only 35% had a MN employment record 7 years after graduation. The percentage of people with no MN employment record hovers around 20%.
...
The biggest different is that graduates from Entirely urban school districts have a significantly lower percentage of individuals with a MN employment record.
...
Next let's break it down by EDR.

The cross tabs indicate a relationship between EDR and percentage of individuals that have a MN employment record for each graduation year time frame. The biggest difference is that EDR 1 has a significantly larger percentage of individuals with no MN employment record, especially 7 years after high school.
...
The table below shows that the percentage of individuals working meaningful employment grows significantly as the years after graduation progress. By 7 years after graduating, 47% have meaningful employment. The percentage of individuals with no MN employment record grows as well - it's 41% of all individuals by seven years after graduating.
...
The cross tabs indicate a relationship. The biggest difference is that the percentage of graduates from entirely urban districts with meaningful employment in the region is significantly lower than the other RUCA categories. In turn, those graduates also have significantly higher percentage of individuals with no MN employment record.
...
Next lets check to see what this looks like by EDR.

The cross tabs indicate a relationship. However, I'm not seeing anything that is worth mentioning.
...
data <- master.11 %>%
  select(edr, meaningful.emp.hs.grad.year) %>%
  filter(meaningful.emp.hs.grad.year != "After 2023")

data.2 <- master.11 %>%
  select(edr, meaningful.emp.hs.grad.year.2) %>%
  filter(meaningful.emp.hs.grad.year.2 != "After 2023")

data.7 <- master.11 %>%
  select(edr, meaningful.emp.hs.grad.year.7) %>%
  filter(meaningful.emp.hs.grad.year.7 != "After 2023")


CrossTable(data$edr, data$meaningful.emp.hs.grad.year, expected = TRUE, prop.t = FALSE, prop.c = FALSE, prop.chisq = FALSE)

CrossTable(data.2$edr, data.2$meaningful.emp.hs.grad.year.2, expected = TRUE, prop.t = FALSE, prop.c = FALSE, prop.chisq = FALSE)

CrossTable(data.7$edr, data.7$meaningful.emp.hs.grad.year.7, expected = TRUE, prop.t = FALSE, prop.c = FALSE, prop.chisq = FALSE)
...
The table below shows that 14% of the individuals in the master dataset had meaningful employment in the same county as their high school the year they graduated. It increased to 19% two years after graduation and then 20% seven years after graduation. These percentages are after filtering out all of the "After 2023" category.

The percentage of individuals with no MN employment record increases from 25% one year after graduating high school to 41% 7 years after graduation.
...
Crosstabs indicate a relationship. The primary difference is that entirely urban graduates have a significanthly higher percentage with individuals not having a MN employment record - 59% by 7 years after high school. They also have a very low percentage with having a location match in their employment.

Entirely rural counties have a significantly higher percentage of individuals that have meaningful employment in the same county as their high school.
...
Crosstabs indicate a relationship. However, the differences are minimal.
...
The chart below shows that 16% of individuals in the main dataset had meaningful employment in the edr the year of graduation. The percent increases to 28% of individuals seven years after graduating. All "After 2023" individuals were taken out.

<br>

```{r summary employment in edr match total}
...
The biggest difference is that the percentage of graduates from entirely urban schools are significantly less likely to have meaningful employment in the same EDR as their high school, while also having a significantly higher percentage that don't have a MN employment record.

Entirely rural individuals tend to not have meaningful employment in the same EDR as their high school.
...
```{r summary edr location match by ruca}
...
```{r summary edr location match by ruca cross tabs}
data <- master.11 %>%
...
EDR 2 has a significantly higher percentage of individuals without meaningful employment in the same EDR as their high school.
...
```{r summary edr location match by edr}
...
```{r summary edr location match by edr cross tabs}
data <- master.11 %>%
...
The chart below shows that 17% of individuals in the main dataset had meaningful employment in the region the year of graduation. The percent increases to 31% of individuals seven years after graduating. All "After 2023" individuals were taken out.
...
Entirely urban graduates have significantly lower percentage with meaningful employment in Northwest, while also have a significantly higher percentage without a MN employment record. IN addition, the more rural a school district is, the higher the percentage of individuals that have meaningful employment in the region.
