...
The original dataset with all MN employment records is huge and bogs down the analysis. So the dataset I'm importing here is from a script I used to filter all employment records to contain PersonID from the Northwest MN graduates.
...
grad.emp.location.match <- grad.locations %>%
  ...
  mutate(region.match = new.mn.emp.record,
         new.region.match = ifelse(region.match == "Meaningful employment" & emp.pr %in% c("Northwest"), "Meaningful emp - match", region.match),
...
About 15% of graduates have meaningful employment in the same county as their high school starting at around 5 years after graduating. The percentage of graduates that have meaningful employment, but not in the same county as their high school is 19% while the percentage of individuals with no MN employment record grows to 36%.
...
At 5 years after graduation, the percentage of graduates that have meaningful employment in their EDR is around 21%. The percentage with meaningful employment outside of their EDR is 27% while 38% have no MN employment record.
...
Just about a quarter of graduates have meaningful employment in the Northwest region by 5 years after graduating high school and about 27% have meaningful employment in MN, but not the region.
