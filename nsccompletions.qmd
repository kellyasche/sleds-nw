...
Alright, now we are going to determine who graduated. Let's see what this data looks like. I'm going to import the entire dataset and then filter by PersonID among the high school graduates from Northwest.
...
nsccompletions.original <- read_csv('/Users/kellyasche/Library/CloudStorage/GoogleDrive-kasche@ruralmn.org/My Drive/Research/FY25-27/FY26/OHE Journey to meaningful employment/Data/SLEDS/NSCCompletions/NCS_Completions.csv') %>%
  rename(PersonID = 1) %>%
...
ps.enrollment <- read_csv("~/Library/CloudStorage/GoogleDrive-kasche@ruralmn.org/My Drive/Research/FY25-27/FY26/OHE Journey to meaningful employment/Data/SLEDS/NSCEnrollment/NSC_Enrollments.csv") %>%
  mutate(PersonID = as.integer(PersonID),
         OPEID = str_pad(OPEID, width = 8, side = "left", pad = "0"),
         OPEID.6 = str_sub(OPEID, 1, 6)) %>%
  drop_na(PersonID) %>%
...
Next we will determine which Northwest MN graduates attended a post-secondary institution in the same project region (Northwest project region).
...
The table below shows that the number of individuals that graduated from a post-secondary institution by 2023 was 38% while 13% were still attending a post-secondary institution.
...
There isn't a terribly large difference, though.
...
The biggest difference is that EDR 2 has a significantly lower percentage of graduates that graduate from a post-secondary institution - 32% compared to 38%-40% for the other EDRs.
...
```{r chart ps grad edr}
ps.grad.edr <- master.10 %>%
  group_by(edr, ps.grad) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  group_by(edr) %>%
  mutate(pct = n / sum(n)) %>%
...
ps.grad.edr.plot <- ggplot(ps.grad.edr, aes(ps.grad, pct, fill = edr, group = edr)) +
...
girafe(ggobj = ps.grad.edr.plot, width_svg = 10, height_svg = 6) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))
```
...
```{r crosstabs ps grad edr}
CrossTable(master.10$edr, master.10$ps.grad, expected = TRUE, prop.t = FALSE, prop.c = FALSE, prop.chisq = FALSE)
```
:::
...
The primary trend is that the average number of years between high school graduation and earning first credential increases as a high school becomes more urban - 3.33 for entirely rural, 3.38 years for town/rural mix students, 3.52 for urban/town/rural mix students and 3.87 years for entirely urban graduates.
...
The ANOVA table indicates a relationships.

The biggest difference is that EDR 2 has a significantly longer duration in which graduates graduate from a post-secondary institution - 3.68 compared to 3.32 for EDR 1 and 3.48 for EDR 4.
...
```{r chart years between grad edr}
years.between.grad.edr <- master.10 %>%
  filter(ps.grad == "Y") %>%
  group_by(edr) %>%
  summarize(n = n(),
            mean = mean(years.between.grads),
...
datatable(years.between.grad.edr, class = "cell-border stripe", filter = "top", rownames = FALSE,
          options = list(columnDefs = list(list(className = "dt-center", targets = 1:5)))) %>%
  formatCurrency(2, "", digits = 0) %>%
  formatCurrency(3, "", digits = 2) %>%
  formatCurrency(7, "", digits = 2)
...
```{r anova years between grad edr}
years.between.grads.summary.edr <- master.10 %>%
  filter(ps.grad == "Y")

years.between.grads.anova.edr <- aov(years.between.grads ~ edr, data = years.between.grads.summary.edr)

summary(years.between.grads.anova.edr)

TukeyHSD(years.between.grads.anova.edr)
...
First, let's look at it from a total dataset. Of the over 60,000 individuals that attended post-secondary in the dataset, over 43% graduated from a public, 4-year college, 34% graduated from a 2-year public college, 11% graduated from a private 4-year, and 10% graduated from a mixture of institution types.
...
1.  Entirely urban:
    ...
    2.  Significantly less likely to earn a credential from a 2-year, public
    3.  Significantly more likely to earn a credential from a private 4-year college
2.  Urban/town/rural mix:
    1.  Significantly more likely to earn a credential from a private, 4-year college.
3.  Town/rural mix
    1.  Significantly more likely to earn a credential from a not-for-profit 4-year
    2.  Significantly less likely to earn a credential from a public, 4-year college
4.  Entirely rural
    1.  Significantly less likely to earn a credential from a 4-year public institution.
    2.  Significantly more likely to earn a credential from a 2-year public institution.
    3.  Significantly less likely to earn a credential from a private 4-year college.
...
1.  EDR 1:
    ...
    3.  Significantly less likely to earn a credential from a private 4-year college.
2.  EDR 2
    1.  Significantly more likely to earn a credential from a 4-year public institution
    2.  Significantly less likely to earn a credential from a 2-year public institution
    3.  Significantly more likely to earn a credential from a private 4-year institution
3.  EDR 4
    1.  Significantly less likely to earn a credential from a 4-year public institution.
    2.  Significantly less likely to earn a credential from a 2-year public institution.
    3.  Significantly more likely to earn a credential from a private 4-year institution
...
```{r chart grad institution sector edr}
grad.sector.edr <- master.10 %>%
  filter(!is.na(ps.grad.InstitutionSector)) %>%
  group_by(edr, ps.grad.InstitutionSector) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  left_join(sectors, by = c("ps.grad.InstitutionSector" = "Code")) %>%
  select(edr, Sector, n) %>%
  ...
  mutate(pct = n / sum(n)) %>%
  ungroup() %>%
  complete(edr, Sector, fill = list(n = 0, pct = 0)) %>%
  select(edr, Sector, pct) %>%
  arrange(pct) %>%
  spread(key = edr, value = pct) 

datatable(grad.sector.edr, class = "cell-border stripe", filter = "top", rownames = FALSE,
...
```{r crosstabs grad sector edr}
grad.sector.summary.edr <- master.10 %>%
  filter(!is.na(ps.grad.InstitutionSector)) %>%
  left_join(sectors, by = c("ps.grad.InstitutionSector" = "Code"))

CrossTable(grad.sector.summary.edr$Sector, grad.sector.summary.edr$edr, expected = TRUE, prop.t = FALSE, prop.c = FALSE, prop.chisq = FALSE)
