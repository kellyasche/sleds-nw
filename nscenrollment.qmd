...
Okay, lets check out the National Student Clearinghouse enrollment data. I will first import the entire dataset, clean it up, and then merge it with the master / Northwest graduate dataset.
...
One thing that's important is to realize that joining the nsc.enrollment dataset with the Northwest graduates dataset means we will have some NAs. Some of the students listed in the nsc.enrollment dataset aren't in the Northwest graduates dataset since they may have graduated before 2008 or didn't actually meet the criteria in the Northwest graduates dataset. Therefore, as we move forward, I will need to make sure to remove those NAs.
...
So there are fewer observations in this dataset than previous subsets. Why? This is due to dropping a PersonID that wasn't in the Northwest graduate dataset. I did not do that for the previous subsets. However, those previous subsets will be filtered down once we join it with the master dataset.
...
Up next we will create a dataset indicating whether a PersonID that graduated from a Northwest MN high school attended a post-secondary institution in their high school's EDR.
...
As expected, there are `r comma(nsc.enrollment.same.pr %>% nrow(), accuracy = 1)` rows and `r comma(nsc.enrollment.same.pr %>% ncol(), accuracy = 1)` columns. This dataset provides whether the post-secondary institution(s) attended were in the same planning region as the high school from which they graduates, outside of the planning region, or both (attended multiple institutions).
...
The chart below provides the percentage of students in the entire dataset that attended each institution sector. By far, a huge majority attend either a public 4-year (47%) or public 2-year (41%).
...
The biggest difference is that EDR 2 attends 4-year public institution at a much higher rate while attending a 2-year public at a much lower rate. The other EDRs were the opposite.
...
```{r chart first college sector edr}
...
```{r crosstabs first college sector edr}
...
The chart below shows that 29% of students graduating from a Northwest MN high school attended a post secondary institution that was located in a county with the same RUCA category as their high school.
...
A significantly lower percentage of entirely rural graduates attended a post-secondary institution inside their RUCA category.
...
The biggest difference is that graduates from EDR 4 were much more likely to attend college in the same EDR.
...
The chart below shows that nearly 40% of students graduating from Northwest MN High schools leave the region to attend post secondary education.
...
The chart below shows that 58% of students graduating from Northwest MN high schools attend a post secondary institution in Minnesota.
...
The biggest difference is that graduates from entirely urban districts attend college in Minnesota at a lower rate than other county groups - 51% compared to 55%+ for the other county groups.
...
EDR 2 has a signifcantly higher percentage of graduates attend college in Minnesota. EDR 2 had 66% of their graduates attend college in Minnesota while 56% of the other EDRs attended in Minnesota.
...
```{r crosstabs attend college MN edr}
...
Outside of Minnesota, Northwest MN graduates attend colleges in North Dakota at the highest rate - 24%.
